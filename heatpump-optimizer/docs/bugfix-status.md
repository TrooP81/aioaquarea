...
| ID | Area | Summary | Status | Files changed | Tests | Notes |
|---|---|---|---|---|---|---|
| CORE-7 | core | `AquareaWrapper` read Panasonic credentials from `.env`, ignoring the Settings tab; failed login crash-looped poller/optimizer | VERIFIED | `packages/core/services/aquarea.py`, `packages/core/settings_service.py`, `packages/core/config.py`, `.env.example`, `README.md` | `tests/test_aquarea_service.py::TestPanasonicCredentialsFromSettings` | Credentials now Settings-only (DB); login is retried on next device access instead of crashing |
| LIB-14 | library | Dashboard showed "Off" while space heating was confirmed: missing `operationStatus` defaults to OFF and `current_action` trusted it | FIXED | `aioaquarea/data.py` | `tests/test_device_manager.py::test_current_action_ignores_defaulted_off_operation_status` | Defaulted OFF now falls through to direction/mode; live row shows `HEATING` |
...
- 2026-09-24 — CORE-7 committed as `05e0b6f`; credentials entered in Settings and polling confirmed.
- 2026-09-24 — LIB-14 FIXED (not yet committed). Library 114 passed; optimizer 736 passed, 35 skipped. Containers rebuilt.
