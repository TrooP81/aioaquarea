...
interface LiveOptimizerStatus {
  active_layer: string;
  learning_mode: { enabled: boolean };
  cop_model: unknown;
  demand_model: unknown;
  thermal_model: unknown;
}

interface LiveThermalStatus {
  current: { outdoor_temp: number };
  predictions: unknown;
...
test.describe("Live API health", () => {
  ...
  test("optimizer status exposes layer + model training state", async ({ request }) => {
    const s = await getJson<LiveOptimizerStatus>(request, "/api/optimizer/status");
  ...
  test("thermal status reports calibrated rates and current temps", async ({ request }) => {
    const t = await getJson<LiveThermalStatus>(request, "/api/thermal/status");
